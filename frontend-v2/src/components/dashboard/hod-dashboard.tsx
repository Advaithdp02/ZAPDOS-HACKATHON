  ...
  useEffect(() => {
    if (user && user.role === 'hod' && user.department) {
      const fetchData = async () => {
        try {
          const students = await api.getStudentProfilesByDepartment(user.department!);
          setDepartmentStudents(students);

          const studentIdsInDept = students.map(s => s.studentId);
          
          const [apps, unapproved] = await Promise.all([
            // This is inefficient. A real backend would have a better query.
            api.getApplications().then(allApps => allApps.filter(app => studentIdsInDept.includes(app.studentId))),
