...
function StatusTimeline({ application }: { application: FullApplication }) {
    ...
    const isOffered = application.currentStatus.toLowerCase() === OFFERED_STATUS.toLowerCase();
    ...
    return (
        <div className="flex items-center space-x-2 md:space-x-4 overflow-x-auto pb-4">
            {statusHierarchy.map((status, index) => {
                const isCompleted = isOffered ? true : index < currentStatusIndex;
                const isActive = isOffered ? (index === statusHierarchy.length -1) : index === currentStatusIndex;
...
export default function ApplicationsPage() {
  ...
  const generateOfferLetterContent = (app: FullApplication) => {
    if (!user) return "";
    return `
Offer Letter
==============
...
Congratulations! We are pleased to offer you the position of ${app.drive.title} at ${app.company.name}.

This is a ${app.drive.employmentType} role.

Your compensation will be ${formatCtc(app.drive.ctc)}.

We were very impressed with your interviews and believe you will be a great asset to our team.

Please review this offer and let us know if you have any questions. We look forward to you joining us.

Sincerely,
The ${app.company.name} Team
    `;
  };

  const handleDownloadOffer = (app: FullApplication) => {
    const content = generateOfferLetterContent(app);
    const blob = new Blob([content], { type: 'text/plain' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    link.download = `Offer_Letter_${app.company.name}.txt`;
              ...
              <div className="flex justify-between items-start">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {app.currentStatus.toLowerCase() === OFFERED_STATUS.toLowerCase() && (
                      <Button onClick={() => handleDownloadOffer(app)}>
                        <Download className="mr-2 h-4 w-4" /> Download Offer
                      </Button>
                    )}
            ...
            <CardContent className="space-y-6">
               <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground">
                    <div className="flex items-center gap-2">
                     ...
                     <div className="flex items-center gap-2">
                        <FileText />
                        <span>{formatCtc(app.drive.ctc)}</span>
                    </div>
               </div>
