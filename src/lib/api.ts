...
export async function register(userData: Omit<User, 'id'>): Promise<User | null> {
    ...
    const userResult = await usersCollection.insertOne({
        ...userData,
        avatarUrl: `https://avatar.vercel.sh/${userData.email}.svg`,
    });

    // If the user is a student, create a corresponding student profile
    if (userData.role === 'student' && userData.department) {
        const studentId = `student-${Math.floor(1000 + Math.random() * 9000)}`;
    ...
    const newUser = await usersCollection.findOne({ _id: userResult.insertedId });
...
export async function updateStudentProfile(userId: string, profileData: Partial<StudentProfile>): Promise<StudentProfile | null> {
    ...
    if (setValues.resumes) {
        setValues.resumes = assignObjectIds(setValues.resumes);
    }
...
export async function createApplication(userId: string, driveId: string, coverLetter: string): Promise<Application> {
    ...
    const newApplicationData = {
        ...
        // Use the first uploaded resume as the one for the application
        resumeUrl: profile.resumes && profile.resumes.length > 0 ? profile.resumes[0].dataUri : '',
...
export async function updateApplicationStatus(applicationId: string, newStatus: string): Promise<Application | null> {
    ...
    if (result) {
        revalidatePath(`/drives/${result.driveId}`);
        revalidatePath('/applications');
