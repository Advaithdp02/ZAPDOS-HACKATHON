...
export async function seedDatabase(client: MongoClient, force = false) {
    ...
    try {
        ...
        // 6. Update drives with application student IDs
        const seededApplications = await applicationsCollection.find().toArray();
        for (const app of seededApplications) {
            if (app.driveId && app.studentId) {
                await drivesCollection.updateOne(
                    { _id: new ObjectId(app.driveId) },
                    { $push: { applications: app.studentId } }
                );
            }
        }
        console.log("Updated drives with student application IDs.");
